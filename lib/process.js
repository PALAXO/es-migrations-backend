                ...
                catShards.forEach((shard) => {
                    if (shard.index.trim().startsWith(Odm.alias) && (shard.prirep === `p`) && shard.node) {
                        ...
                        if (hostWorkers.has(hostname)) {
                            shardWorkers = hostWorkers.get(hostname);
                        } else {
                            //This shouldn't happen
                            shardWorkers = hostWorkers.get(`*default*`);
                        }

                        nodeWorkers.set(hostname, shardWorkers);
                    }
                });
                if (nodeWorkers.size <= 0) {
                    //This shouldn't happen
                    nodeWorkers.set(`*default*`, hostWorkers.get(`*default*`));
                }

                let targetWorkers = 0;
                nodeWorkers.forEach((shardWorkers) => {
                    targetWorkers += shardWorkers;
                });

                const workerConcurrency = (_.isInteger(workerNumbers?.[1])) ? workerNumbers[1] : PROCESS.workerThreadConcurrency;
                const physicalWorkers = (_.isInteger(workerNumbers?.[0])) ? workerNumbers[0] : Math.max(1, Math.min(Math.ceil(targetWorkers / workerConcurrency), os.cpus().length));

                if (!finalResult || (finalResult[0] > physicalWorkers)) {
                    finalResult = [physicalWorkers, workerConcurrency];
                }
            }
...
async function _safeCreateIndex(MyOdm, mySettings, updatedSettings, myMapping, iteration = 0, indexRetry = 0) {
    const knownForbiddenItems = [`index.creation_date`, `index.resize`, `index.provided_name`, `index.routing`, `index.uuid`, `index.version`, `index.routing_partition_size`];
