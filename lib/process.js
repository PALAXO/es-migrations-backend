...
import { SYNCHRONISATION_TYPES } from './synchronisationTypes.js';
...
const PROCESS = nconf.get(`options:parallelization:process`);
...
process.on(`message`, async (value) => {
    try {
        if (value?.type === `__run`) {
            ...
            if (data.metadata.migrationsConfig) {
                logger.setLoggerConfig(data.metadata.migrationsConfig, data.metadata?.options?.loggerUid);
            }
            ...
            await elastic.createElastic(data.metadata.tenant, data.metadata.esHost, data.metadata.indicesInfo, data.metadata.elasticsearchConfig, data.metadata.options?.loggerUid);
            optimizations.setRestrictions(data.restrictions);
            optimizations.setAverageDocumentSizes(data.averageDocumentSizes);

            const checkPath = pathToFileURL(data.metadata.migrationPath).toString();
            registerHooks({
                resolve: (specifier, context, nextResolve) => {
                    const autoPath = nextResolve(specifier, context);
                    if (autoPath?.url === checkPath) {
                        autoPath.url = pathToFileURL(path.join(import.meta.dirname, `Migration.js`)).toString();
                    }
            ...
            for (let migrationIndex = 0; migrationIndex < data.migrations.length; migrationIndex++) {
                ...
                const migrationImport = (await import(pathToFileURL(migrationData.filepath))).default;
                migrationData.Constructor = _.castArray(migrationImport)[migrationData.classIndex];
            }
...
class MigrationProcess {
     ...
     * @param migrations {Array<{Constructor: Function, filepath: string, classIndex: number, version: string, versionNumbers: string}>}
    ...
    constructor(type, index = void 0, inputIndices = [], outputIndices = [], migrations = [], backgroundToProcessData = []) {
        ...
        for (let migrationIndex = 0; migrationIndex < migrations.length; migrationIndex++) {
            ...
            const myMigration = new migrationData.Constructor(migrationData.version, migrationData.versionNumbers, void 0);
        ...
        for (let workerIndex = 0; workerIndex < physicalWorkerCount; workerIndex++) {
            const worker = new Worker(path.join(import.meta.dirname, `worker.js`), {
                ...
                workerData: {
                    ...
                    migrationsConfig: metadata.migrationsConfig,
                    elasticsearchConfig: metadata.elasticsearchConfig,
    ...
    _createUtils(MyOdm = void 0) {
        for (let migrationIndex = 0; migrationIndex <  this._migrations.length; migrationIndex++) {
            ...
            if (myMigration._info.type === SYNCHRONISATION_TYPES.BULK || myMigration._info.type === SYNCHRONISATION_TYPES.DOCUMENTS) {
                /**
                 * Creates a new document, if ID is specified, it must not exist yet
                ...
                /**
                 * Creates a new document, Rewrites the original one, when exists
                 * @param source {{}} Document object
    ...
    async _prepareOutputIndex(MyOdm) {
        ...
        if (requiresReindex) {
            ...
            outputIndex = await elastic.safeCreateIndex(MyOdm, mySettings, updatedSettings, myMapping);
        }
     ...
     * User available number of workers is the multiplication of these numbers.
    ...
    async _getWorkerThreadCount() {
        if (this._type === SYNCHRONISATION_TYPES.DOCUMENTS) {
            ...
            //Find the best worker thread count for each output ODM
            ...
            for (const Odm of outputOdms) {
                ...
                const ramLimit = Math.floor(os.totalmem() / (PROCESS.ramPerWorkerGiB * 1024 * 1024 * 1024));
