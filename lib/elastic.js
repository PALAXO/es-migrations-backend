 ...
 * @returns {Promise<*>}
 */
async function callEs(path, body, options = void 0, TypedOdm = void 0, counter = 0) {
    try {
        ...
        if (options?.asStream) {
            return await parseStream(result, TypedOdm);
        } else {
            return result;
        }
        ...
        if (e.statusCode === 429 && counter < RETRIES.maxRetries) {
            ...
            return callEs(path, body, options, TypedOdm, counter);
        } else {
            throw e;
        }
    }
...
async function migrateToAliases(versionString) {
    ...
    for (const Odm of existingModels) {
        ...
        if (realIndex === Odm._alias) {
            ...
            const newIndex = await Odm.cloneIndex();
 ...
 * @param TypedOdm ODM model used in searches, used for updating document sizes
 * @returns {Promise<{}>} Response object
 */
async function parseStream(esResult, TypedOdm = void 0) {
    let totalLength = 0;
    ...
    if (esResult.statusCode >= 400) {
        ...
        optimizations.updateDocumentSize(TypedOdm, totalLength, body?.hits?.hits?.length);
