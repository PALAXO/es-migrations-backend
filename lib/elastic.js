...
/**
 * name -> workerIndex -> type
 * @type {Record<string, Map<number, {normal, restricted}>>}
 */
const workersClassCache = {};
 ...
 * @returns {*}
 */
function _createClass(indexName, tenant, workerIndex = void 0, workerCount = void 0) {
    return class MyClass extends esOdm.createClass(indexName, void 0, tenant) {
        ...
        //Workers' alter search
        static _alterSearch(query) {
            if (!_.isNil(workerIndex) && !_.isNil(workerCount)) {
                if (workerCount <= 1) {
                    return query;

                } else if (_.isNil(query) || (_.isObject(query) && _.isEmpty(query))) {
                    return {
                        query: {
                            match_all: {}
                        },
...
/**
 * Returns ODM for worker. This ODM has rewritten "_alterSearch" function to use the correct slice
 * @param indexName {string}
 * @param workerIndex {number}
 * @param workerCount {number}
 ...
 * @returns {*}
 */
function getWorkerOdm(indexName, workerIndex, workerCount, isNormal = true) {
    if (!workersClassCache[indexName]) {
        workersClassCache[indexName] = new Map();
    }
    const workerClassCache = workersClassCache[indexName];
    if (!workerClassCache.has(workerIndex)) {
        const BaseClass = this.odms[indexName];
        if (!BaseClass) {
            throw Error(`Internal error - cannot find base class for index name '${indexName}'.`);
        }
        const MyClass = _createClass(BaseClass._name, BaseClass._tenant, workerIndex, workerCount);
        MyClass._maxBulkSize = this.odms[indexName]._maxBulkSize;

        workerClassCache.set(workerIndex, {
            normal: MyClass,
            restricted: MyClass.immediateRefresh(false)
        });
    }

    const workerCache = workerClassCache.get(workerIndex);
    return (isNormal) ? workerCache.normal : workerCache.restricted;
}
...
async function callEs(path, body, options = void 0, counter = 0) {
    try {
        ...
        if (e.statusCode === 429 && counter < RETRIES.maxRetries) {
            const debugLimit = Math.min(3, Math.floor(RETRIES.maxRetries / 2));
