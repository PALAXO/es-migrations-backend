...
const SCROLL_TIMEOUT_SECONDS = nconf.get(`es:scrollTimeoutSeconds`);
...
async function createElastic(tenant, host, indicesInfo) {
    ...
    esOdm.setClient(host, {
        scrollTimeoutSeconds: SCROLL_TIMEOUT_SECONDS
    });
 ...
 * @returns {Promise<*>}
 */
async function callEs(path, body, options = void 0, MyOdm = void 0, counter = 0) {
    try {
        ...
        if (options?.asStream) {
            return await parseStream(result, MyOdm);
        } else {
            return result;
        }
        ...
        if (e.statusCode === 429 && counter < RETRIES.maxRetries) {
            ...
            return callEs(path, body, options, MyOdm, counter);
        } else {
            throw e;
        }
    }
 ...
 * @param MyOdm ODM model used in searches, used for updating document sizes
 * @returns {Promise<{}>} Response object
 */
async function parseStream(esResult, MyOdm = void 0) {
    let totalLength = 0;
    ...
    if (esResult.statusCode >= 400) {
        ...
        optimizations.updateDocumentSize(MyOdm, totalLength, body?.hits?.hits?.length);
