...
class ScriptWrapper {
     ...
     * @param key {string | { isJavascript: boolean, alias: String, type: String }}
    ...
    constructor(key, context) {
        if (!context) {
            ...
            this.context = {
                ...
                alias: key.alias,
                type: (_.isUndefined(key.type)) ? null : key.type,
    ...
    /**
     * Returns JS function or painless code
     * @returns {string | Function}
     */
    _getResult() {
        if (this.__isJavaScript()) {
            return new Function(`doc`, `const root = doc; ${this.context.code}`);
        } else {
    ...
    get utils() {
        ...
        return {
            ...
            /**
             * Returns ScriptWrapper with document index type (or null)
             * @returns {ScriptWrapper} New string or null ScriptWrapper instance
             * @private
             */
            get _type() {
                const valueKey = self.__generateKeyName(`type`);
                if (self.__isJavaScript()) {
                    self.__addCode(`let ${valueKey} = ${_.isNull(self.context.type) ? `null` : `'${self.context.type}'`};`);
                } else {
                    self.__addCode(`def ${valueKey} = ${_.isNull(self.context.type) ? `null` : `'${self.context.type}'`};`);
                }
                return new ScriptWrapper(valueKey, self.context);
            },
