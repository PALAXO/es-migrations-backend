...
/**
 * Merge given note objects into this thread
 * @param messages {Record<string, {version: string, messages: Array<string>}>}
 */
function mergeNotes(messages) {
    if (!_.isEmpty(messages)) {
        Object.entries(messages).forEach(([versionNumbers, data]) => {
            if (!global.MESSAGES[versionNumbers]) {
                global.MESSAGES[versionNumbers] = {
                    version: data.version,
                    messages: []
                };
            }

            global.MESSAGES[versionNumbers].messages.push(...data.messages);
        });
    }
}
