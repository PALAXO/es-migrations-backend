...
class Migrate {
    ...
    constructor(repository) {
        ...
        /** @type {{tenant: string, esHost: string, indicesInfo: {INDICES: Record<string, {name: string, allowed: {initial: boolean, versions: Array<{from: string, allowed: boolean}>}, types: {initial: boolean, versions: Array<{from: string, types: boolean}>}, maxBulkSize: number}>}, migrationPath: string, migrationsConfig: {}, elasticsearchConfig: {}}} */
     ...
     */
    async initialize(tenant, esHost, indicesInfo, migrationPath, migrationsConfig = void 0, elasticsearchConfig = void 0)  {
        if (migrationsConfig) {
            logger.setLoggerConfig(migrationsConfig);
        }

        this._metadata = {
            tenant, esHost, indicesInfo, migrationPath, migrationsConfig, elasticsearchConfig
        };
    ...
    async restore() {
        ...
        const snapshot = await this._restoreLatestSnapshot();
        if (snapshot) {
     ...
     * @param targetMeta {{}}
     * @param additional {{processInfo: [number, number, number]}}
    ...
    async _restoreLatestSnapshot() {
        ...
        const mySnapshots = allSnapshots.filter((snap) => snap.snapshot.split(`_`).length === 3
            ...
            && _.isInteger(parseInt(snap.snapshot.split(`_`)[2], 10)));
        ...
        //Restore dev
        await this._restoreSnapshot(latestSnapshot.snapshot);
    ...
    async _migrate(nodes, targetMeta) {
        ...
        let backupInfo;
        try {
            backupInfo = await this._backUp(nodes);
        } catch (e) {
        ...
        await this._cleanUp(isSuccess, backupInfo);
        await elastic.unlockTenant(this._tenant);
     ...
     * @returns {Promise<{usageMap: Record<string, boolean>, snapshot: string}>}
     */
    async _backUp(nodes = void 0) {
        if (!_.isEmpty(this._snapshot)) {
            //Already snapshoted
            return void 0;
            ...
            this._snapshot = await this._makeSnapshot();
            ...
            const usageMap = (BACKUP.onlyUsedIndices) ? await _getIndexUsage(this._tenant, nodes) : void 0;
            const snapshot = await this._makeSnapshot(usageMap);
            logger.info(`Data backed up.`);
     ...
     */
    async _makeSnapshot(usageMap = void 0) {
        ...
        if (_.isEmpty(indices)) {
            ...
            return this._createSnapshot(indices);
        }
    }
     ...
     * @param backupInfo {{usageMap: Record<string, boolean>, snapshot: string}}
     * @returns {Promise<void>}
     */
    async _cleanUp(isSuccess, backupInfo = void 0) {
        logger.info(`Running clean up...`);

        if (!isSuccess) {
            const indices = (!_.isEmpty(backupInfo?.usageMap)) ? await this._getUsedIndices(backupInfo.usageMap) : `${this._tenant}_*`;
            if (!_.isEmpty(indices)) {
            ...
            const snapshot = this._snapshot || backupInfo.snapshot;
    ...
    /**
     * Checks which indices have been used during migration
     * @param usageMap {Record<string, boolean>}
     * @returns {Promise<Array<string>>}
     */
    async _getUsedIndices(usageMap) {
...
async function _computeHostsWorkers() {
    ...
    hostWorkers.set(`*default*`, _getWorkerCountForHost(4, 4 * 1024 * 1024 * 1024));
...
function _getWorkerCountForHost(threads, heap) {
    ...
    const heapLimit = Math.floor(heap / (PROCESS.heapPerESThreadGiB * 1024 * 1024 * 1024));
