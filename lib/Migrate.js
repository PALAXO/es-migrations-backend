...
class Migrate {
    ...
    async _restoreSnapshot(snapshot) {
        ...
        if (clusterStatus?.status === `red` || clusterStatus?.status === `RED`) {
            console.log(`!!! Warning: Cluster status is in RED state. The restore task may not be able to finish, unless primary shards recover. !!!`);
            console.log(`You may need to end this script and restore ES manually. After the state is yellow or green, you can run this script again and choose to restore the backup.`);
        }
...
async function _computeHostsWorkers() {
    ...
    hostWorkers.set(`*default*`, _getWorkerCountForHost(8, 12 * 1024 * 1024 * 1024));
