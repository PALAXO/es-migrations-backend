...
async function migrate(self, nodes, targetMeta) {
    ...
    try {
        ...
        optimizations.createCache(Object.values(metaMapping)[0]?.mappings?._meta?._optimizations);
        ...
        //Update target version and cache
        ...
        allIndices.forEach((myIndex) => {
            if (_.isEmpty(lockPoints[myIndex])) {
                lockPoints[myIndex] = {
                    ...
                    readingNodes: [],       //Nodes that reads from source node
                };
            }
        });
