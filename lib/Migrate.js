...
class Migrate {
    ...
    async _backUp(nodes = void 0) {
        if (!_.isEmpty(this._fullSnapshot)) {
            ...
            const partialSnapshot = await this._makeSnapshot(usageMap);
            logger.info(`Data backed up.`);
            ...
            do {
                try {
                    ...
                    if (retry) {
                        ...
                        logger.debug(e);
                        logger.info(`Waiting before retry (${counter}/${RESTORE.maxRetries})`);
    ...
    async _restoreSnapshot(snapshot) {
        ...
        await this._deleteExistingIndices(indices);
     ...
     * @returns {Promise<void>}
     */
    async _deleteExistingIndices(indices) {
        if (indices.length > 0) {
            //Delete existing indices

            const toCheckAliases = [];
            for (const index of indices) {
                if (index.startsWith(`${this._tenant}_`)) {
                    toCheckAliases.push(index.replace(/([^-]+)(?:-[^_]+)?(_.+)?/g, `$1$2`));
                }
            }

            if (toCheckAliases.length > 0) {
                let toDeleteIndices = [];

                //Check indices for aliases
                ...
                for (const existingIndex of existingIndices) {
                    if (existingIndex.startsWith(`${this._tenant}_`)) {
                        const alias = existingIndex.replace(/([^-]+)(?:-[^_]+)?(_.+)?/g, `$1$2`);
                        if (toCheckAliases.includes(alias)) {
                            toDeleteIndices.push(existingIndex);
                        }
                    }
                }
