...
class Migrate {
    ...
    async initialize(tenant, esHost, indicesInfo, migrationPath, migrationsConfig = void 0, elasticsearchConfig = void 0, options = void 0)  {
        ...
        this._tenant = tenant;
        return ((await import(`./Migration.js`)).default);
    }
    ...
    async _migrate(nodes, targetMeta) {
        ...
        try {
            ...
            //Update outdated indices
            await elastic.updateIndices();
            ...
            for (let j = i + 1; j < migrationBulk.length; j++) {
                ...
                if (!bufferIndexOutputIndices.some((element) => nextMigrationInputIndices.includes(element)) &&
                    ...
                    if (nextMigration._info.index === index) {
                        ...
                        bufferIndexOutputIndices = _.uniq([...bufferIndexOutputIndices, ...nextMigration._info.outputIndices]);                                     //A
                        bufferAllIndices = _.uniq([...bufferAllIndices, ...nextMigration._info.inputIndices, ...nextMigration._info.outputIndices]);                    //B
                        bufferInputOutputIndices = _.uniq([...bufferInputOutputIndices, ...nextMigration._info.inputIndices, ...nextMigration._info.outputIndices]);    //C
