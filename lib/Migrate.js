...
class Migrate {
    ...
    async migrate(migrations, currentVersionString, targetMeta) {
        ...
        identifyDependencies(migrations);
 ...
 * @param migrations {Array<Migration>}
 */
function identifyDependencies(migrations) {
...
async function migrate(self, nodes, targetMeta) {
    ...
    try {
        ...
        await _runGraph(graph, self._metadata);
...
async function _backUp(self, nodes = void 0) {
    if (!_.isEmpty(self._snapshot)) {
        ...
        const usageMap = (BACKUP.onlyUsedIndices) ? await _getIndexUsage(self._tenant, nodes) : void 0;
 ...
 * @param metadata {{tenant: string, esHost: string, indicesInfo: {INDICES: Record<string, {name: string, types: {initial: boolean, versions: Array<{from: string, types: boolean}>}, maxBulkSize: number}>}, migrationPath: string}} Migration metadata
 * @returns {Promise<void>}
 */
async function _runGraph(graph, metadata) {
    ...
    try {
        do {
            ...
            while ((nodesQueue.length > 0) && (runningNodes.length < PARALLELIZATION.targetProcesses)) {
                //Run node
                const node = nodesQueue.shift();
                node.run(runningNodes, metadata);
            }
