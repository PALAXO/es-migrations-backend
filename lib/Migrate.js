...
async function _computeHostsWorkers() {
    ...
    if (nodesInfo.nodes) {
        for (const node of Object.values(nodesInfo.nodes)) {
            if (_.isArray(node?.roles) && node.roles.some((role) => role.includes(`data`))) {
                ...
                if (_.isString(node?.name) && _.isInteger(node?.thread_pool?.write?.size)) {
                    const targetWorkers = node.thread_pool.write.size;
    ...
    hostWorkers.set(`*default*`, PROCESS.defaultNodeThreads);
