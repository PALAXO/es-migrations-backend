...
async function _backUp(self, nodes = void 0) {
    if (!_.isEmpty(self._fullSnapshot)) {
        ...
        const partialSnapshot = await _snapshot(self, usageMap);
        logger.info(`Data backed up.`);
...
async function _restoreSnapshot(self, snapshot) {
    ...
    await _deleteExistingIndices(self, indices);
 ...
 * @private
 */
async function _deleteExistingIndices(self, indices) {
    if (indices.length > 0) {
        //Delete existing indices

        const toCheckAliases = [];
        for (const index of indices) {
            if (index.startsWith(`${self._tenant}_`)) {
        ...
        if (toCheckAliases.length > 0) {
            ...
            for (const existingIndex of existingIndices) {
                if (existingIndex.startsWith(`${self._tenant}_`)) {
