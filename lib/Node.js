...
class Node {
    ...
    async _run() {
        ...
        logger.debug(`Starting node with migrations [${this._migrations.map((migration) => migration.version).join(`, `)}].`);
    ...
    async _prepareMigrations(MainOdm, indexMapping) {
        ...
        for (const migration of this._migrations) {
            if (_.isArray(migration.__updatedDocuments)) {
                ...
                const multipleRecords = migration.__updatedDocuments.some((updatedDocument) => {
                    return (migration.__updatedDocuments.filter((doc) => (doc.alias === updatedDocument.alias && doc.id === updatedDocument.id)).length !== 1);
                });
                if (multipleRecords) {
                    throw Error(`'${migration.version}': You have specified multiple updateDocument or forceCreateDocument functions for the same document.`);
                }
            }
        }
    }
    ...
    /**
     * Migrates virtual documents. Virtual documents are the documents that don't exist in ES yet, but has been specified to be created.
     * @param MainOdm
     * @param indexMapping {{}}
     * @param virtualDocuments {{}}
     * @param toDelete {{}}
