...
class Migration {
    ...
    createDocument(source, id = void 0) {
        ...
        this.__createdDocuments.push({ source, id, isForced: false });
    }
    ...
    updateDocument(func, fallbackSource = void 0, id) {
        if (this._info.type !== SYNCHRONISATION_TYPES.DOCUMENTS) {
        ...
        } else if (_.isNil(func) || !_.isFunction(func) || _.isNil(this[func.name])) {
            throw Error(`'${this.version}': You have to point to the update method for 'updateDocument' function.`);
            ...
            throw Error(`'${this.version}': Fallback source has to be a nil value or an object in 'updateDocument' function.`);
        } else if (this.__isInitialized) {
            throw Error(`'${this.version}': Function 'updateDocument' can be used only in 'beforeAll' function.`);
        }

        if (this.__updatedDocuments.has(id)) {
            throw Error(`'${this.version}': You have specified multiple 'updateDocument' functions for the document: '${id}'`);
        } else {
            this.__updatedDocuments.set(id, { functionName: func.name, fallbackSource, isUsed: false, isForced: false });
        }
    }
    ...
    forceCreateDocument(source, id) {
        if (this._info.type !== SYNCHRONISATION_TYPES.DOCUMENTS) {
            ...
            throw Error(`'${this.version}': Function 'forceCreateDocument' can be used only in 'beforeAll' function.`);
        }

        if (this.__updatedDocuments.has(id)) {
            throw Error(`'${this.version}': You have specified multiple 'forceCreateDocument' functions for the document: '${id}'`);
        } else {
            this.__createdDocuments.push({ source, id, isForced: true });
            this.__updatedDocuments.set(id, { functionName: this.__internalDeleteDocument.name, fallbackSource: void 0, isUsed: false, isForced: true });
        }
    }
