...
utils.promisifyThreadCommunication(parentPort, async (message) => {
    try {
        ...
        switch (message.type) {
            case `__initialize`: {
                await elastic.createElastic(workerData.tenant, workerData.esHost, workerData.indicesInfo, workerData.elasticsearchConfig, workerData.options?.loggerUid);
                ...
                const checkPath = pathToFileURL(workerData.migrationPath).toString();
                registerHooks({
                ...
                for (let i = 0; i < workerData.concurrency; i++) {
                    ...
                    //Fill constructors
                    for (let migrationIndex = 0; migrationIndex < message.data.migrations.length; migrationIndex++) {
                        const migrationData = message.data.migrations[migrationIndex];
...
class MigrationWorker {
    /**
     *
     * @param data {{migrations: Array<{Constructor: Function, filepath: string, classIndex: number, version:string, versionNumbers: string}>,
    ...
    constructor(data, workerIndex) {
        ...
        this.MyOdm = (this.index) ? elastic.restrictedOdms[data.index] : void 0;
        ...
        for (let migrationIndex = 0; migrationIndex < data.migrations.length; migrationIndex++) {
            ...
            for (const name of Object.keys(myMigration.ODM)) {
                if (restrictedIndices.includes(name)) {
                    myMigration.ODM[name] = elastic.restrictedOdms[name];
                }
            }
    ...
    async migrateExistingDocuments(data) {
        ...
        if (!isTerminated) {
            const body = {
                ...
                slice: (workerData.workerCount > 1) ? {
                    id: this.workerIndex,
                    max: workerData.workerCount
                } : void 0
            };
    ...
    _createUtils(myMigration, migrationIndex) {
        ...
        myMigration.UTILS.sliceQuery = (query = void 0) => {
            let myQuery = query;
            if (!_.isNil(query)) {
                myQuery = _.cloneDeep(query);
            }

            if (workerData.workerCount <= 1) {
                return myQuery;

            } else if (_.isNil(myQuery) || (_.isObject(myQuery) && _.isEmpty(myQuery))) {
                return {
                    query: {
                        match_all: {}
                    },
                ...
                throw Error(`Wrong search query has been specified.`);
            }
        };
