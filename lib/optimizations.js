...
async function getBulkSize(MyOdm) {
    if (!OPTIMIZATIONS.targetBulkSize.enabled) {
        ...
        if (!optimizations.averageDocumentSizes[MyOdm.alias]) {
            const randomDocs = await MyOdm.search({}, 0, OPTIMIZATIONS.targetBulkSize.sampleSize, true);
            if (randomDocs.length > 0) {
                ...
                _updateDocumentSize(MyOdm, (byteSize / sources.length));
            }
        }
 ...
 */
function updateDocumentSize(MyOdm = void 0, byteSize, docCount) {
    if (!MyOdm) {
        return;
    }
    ...
    if (docCount > 0) {
        ...
        _updateDocumentSize(MyOdm, ((totalScale * originalDocumentSize) + ((1 - totalScale) * batchDocumentSize)));
    }
}
