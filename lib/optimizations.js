...
async function updateDocumentSize(MyOdm = void 0, dataArray) {
    ...
    if (dataArray.length > 0) {
        ...
        const indexDocuments = Math.max(await MyOdm.count(), 1);
        const sizeScale = Math.min(dataArray.length / indexDocuments, 1);

        //Compute new document size
        ...
        const newDocumentSize = ((sizeScale * batchDocumentSize) + ((1 - sizeScale) * originalDocumentSize));
