        ...
        for (const TypedOdm of allTypes) {
            if (!optimizations.averageDocumentSizes[TypedOdm._alias]) {
                ...
                if (randomDocs.length > 0) {
                    ...
                    _updateDocumentSize(TypedOdm, (byteSize / randomDocs.length));
                }
            }
 ...
 */
async function updateDocumentSize(TypedOdm = void 0, dataArray) {
    if (!TypedOdm) {
        return;
    ...
    if (dataArray.length > 0) {
        ...
        const indexDocuments = Math.max(await TypedOdm.count(), 1);
        ...
        _updateDocumentSize(TypedOdm, newDocumentSize);
    }
}
