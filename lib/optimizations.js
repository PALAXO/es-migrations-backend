...
function getCache() {
    ...
    if (!_.isEmpty(myOptimizations.averageDocumentSizes)) {
        //Sort average document sizes by the key
        const keys = Object.keys(myOptimizations.averageDocumentSizes);
        keys.sort((a,b) => a.localeCompare(b, `en`));

        const sortedAverageDocumentSizes = {};
        keys.forEach((key) => sortedAverageDocumentSizes[key] = myOptimizations.averageDocumentSizes[key]);
        myOptimizations.averageDocumentSizes = sortedAverageDocumentSizes;
    }
...
async function getBulkSize(MyOdm) {
    if (!OPTIMIZATIONS.targetBulkSize.enabled) {
        ...
        if (!optimizations.averageDocumentSizes[MyOdm.alias]) {
            ...
            if (randomDocs.length > 0) {
                ...
                const fullSize = (new TextEncoder().encode(JSON.stringify(sources))).length;
                const bracketSize = (new TextEncoder().encode(`[]`)).length;
                const commaSize = ((new TextEncoder().encode(`,`)).length) * (randomDocs.length - 1);
                const byteSize = fullSize - bracketSize - commaSize;

                _updateDocumentSize(MyOdm, (byteSize / randomDocs.length));
            }
        }

        if (optimizations.averageDocumentSizes[MyOdm.alias]) {
            return _getBulkSize(MyOdm);
        } else {
...
async function updateDocumentSize(MyOdm = void 0, dataArray) {
    ...
    if (dataArray.length > 0) {
        const fullSize = (new TextEncoder().encode(JSON.stringify(dataArray))).length;
        const bracketSize = (new TextEncoder().encode(`[]`)).length;
        const commaSize = ((new TextEncoder().encode(`,`)).length) * (dataArray.length - 1);
        const byteSize = fullSize - bracketSize - commaSize;

        //It is a difference whether we compute average document size from 1 or from 10000 documents
        //We consider full scale (=== 1) as total number of document in the index
        const indexDocuments = await MyOdm.count();
        const sizeScale = dataArray.length / indexDocuments;
        const lowPassScale = OPTIMIZATIONS.targetBulkSize.lowPassCoefficient ?? 0.8;
        //We power the low pass scale using size scale
        const totalScale = Math.pow(lowPassScale, sizeScale);

        //Compute new document size using low pass filter
        const batchDocumentSize = Math.floor(byteSize / dataArray.length);
        const originalDocumentSize = optimizations?.averageDocumentSizes?.[MyOdm.alias] ?? batchDocumentSize;
        const newDocumentSize = ((totalScale * originalDocumentSize) + ((1 - totalScale) * batchDocumentSize));

        _updateDocumentSize(MyOdm, newDocumentSize);
    }
}
 ...
 * @private
 */
function _getBulkSize(MyOdm) {
    const documentSize = optimizations.averageDocumentSizes[MyOdm.alias];
...
/**
 * Updates average document size of given ODM
 * @param MyOdm
 * @param documentSize {number} Average size of a single document
 * @private
 */
function _updateDocumentSize(MyOdm, documentSize) {
    optimizations.averageDocumentSizes[MyOdm.alias] = Math.max(Math.floor(documentSize), 1);
}
